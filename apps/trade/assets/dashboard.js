(() => {
  ...
  function card(r){
    ...
    return `<article class="enquiry-card ${r.seed?'':'session-record'}" data-ref="${escapeHtml(r.ref)}">
      ...
      <div><span class="priority ${urgent?'urgent':'normal'}">${escapeHtml(r.urgency||'Normal')}</span><small class="record-status ${r.status==='CONTACTED'?'contacted':''}">${escapeHtml(r.status||'NEW')}</small></div>
  ...
  function openQuote(r){
    $('#quote-ref').value=r.ref;$('#quote-description').value=r.jobType + ' — ' + (r.details||'');
    ['labour','materials','callout','other'].forEach(k=>$('#quote-'+k).value='0');$('#quote-notes').value='';calcTotal();openDialog($('#quote-dialog'));
  }
  function calcTotal(){const total=['labour','materials','callout','other'].reduce((n,k)=>n+(parseFloat($('#quote-'+k).value)||0),0);$('#quote-total').textContent='£'+total.toFixed(2);return total}
  ['labour','materials','callout','other'].forEach(k=>$('#quote-'+k).addEventListener('input',calcTotal));
  document.addEventListener('click',e=>{
    const btn=e.target.closest('[data-action]');if(!btn)return;const card=btn.closest('.enquiry-card');const r=getRecord(card?.dataset.ref);if(!r)return;
  ...
  $('#quote-builder').addEventListener('submit',e=>{e.preventDefault();const quotes=loadQuotes();quotes.unshift({ref:$('#quote-ref').value,description:$('#quote-description').value,total:calcTotal(),notes:$('#quote-notes').value,createdAt:new Date().toISOString()});saveQuotes(quotes);closeDialog($('#quote-dialog'));render();showView('quotes');});
  function showView(view){$$('.dashboard-view').forEach(p=>p.hidden=p.dataset.viewPanel!==view);$$('.desk-nav').forEach(b=>b.classList.toggle('active',b.dataset.view===view));$('#view-title').textContent=({dashboard:'Dashboard',enquiries:'New Enquiries',quotes:'Quotes',followups:'Follow-ups',settings:'Settings'})[view]||'Dashboard';}
